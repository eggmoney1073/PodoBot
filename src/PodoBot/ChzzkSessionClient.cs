...
public sealed class ChzzkSessionClient : IAsyncDisposable
{
    ...
    public async Task ConnectAsync(string accessToken)
    {
        ...
        try
        {
            ...
            var openPacket =
                await ReceiveTextAsync(socket, connectTimeout.Token);

            if (string.IsNullOrWhiteSpace(openPacket)
                || openPacket[0] != '0')
            {
                throw new InvalidOperationException(
                    "치지직 Engine.IO handshake를 받지 못했습니다.");
            }

            ReadEngineOpenPacket(openPacket);

            // Socket.IO 2.x root namespace:
            // the server sends the initial CONNECT packet ("40").
            // Do not send "40" from the client.
        ...
        CancellationToken cancellationToken)
    {
        ...
        switch (packet[0])
        {
            ...
            case '2':
                // Defensive handling if a ping is received.
                await SendTextAsync(
                    socket,
                    "3" + packet[1..],
                    cancellationToken);
            ...
            case '3':
                // Pong for Engine.IO v3 ping.
                return;

            case '4':
                await HandleSocketIoPacketAsync(
                    packet[1..]);
                return;
        }
    }

    private async Task HandleSocketIoPacketAsync(
        string packet)
    {
        if (string.IsNullOrEmpty(packet))
            return;

        // Socket.IO EVENT on root namespace.
        if (packet[0] != '2')
            return;
        ...
        JsonElement data)
    {
        ...
        {
            var sessionKey =
                data.GetProperty("data")
                    .GetProperty("sessionKey")
                    .GetString() ?? "";

            await _api.SubscribeChatAsync(
                _accessToken,
                sessionKey);
    ...
    private static ChatEvent ParseChat(
        JsonElement data)
    {
        var profile =
            data.TryGetProperty(
