...
public sealed class RepeatingMessageService : IAsyncDisposable
{
    ...
    public RepeatingMessageService(LocalDataStore store, Func<string, Task> send)
    {
        _store = store;
        _send = send;
    }
    ...
    private async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        while (await timer.WaitForNextTickAsync(cancellationToken))
        {
            ...
            var active = _store.Data.RepeatingMessages.Where(x => x.Enabled).ToArray();

            foreach (var item in active)
            {
                if (string.IsNullOrWhiteSpace(item.Message)) continue;

                var interval = TimeSpan.FromMinutes(Math.Max(0.1, item.IntervalMinutes));
                ...
                try
                {
                    ...
                    Log?.Invoke($"반복 안내 전송: {item.Message}");
                }
                catch (Exception ex)
                {
                    Log?.Invoke($"반복 안내 전송 실패: {ex.Message}");
                }
            }
